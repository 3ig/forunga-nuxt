...
export default {
    async asyncData ({ app, store, params, route }) {
        try {
            ...
            let preview = await app.$axios.get(`${store.state.wordpressAPI}/wp/v2/posts/${route.query.preview_id}?_wpnonce=${route.query.preview_nonce}&_embed`, { credentials: 'include' })
            // console.log(preview)
